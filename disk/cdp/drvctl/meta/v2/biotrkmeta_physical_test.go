...
func TestPhysicalDrive(t *testing.T) {
	...
	// 3. 对齐到扇区
	sectorSize := int64(disk.LogicalSectorSize)
	metaStart = (metaStart / sectorSize) * sectorSize
	...
	t.Run("Create", func(t *testing.T) {
		...
		t.Logf("  Device: type=%v id=%s extents=%d",
			pd.Type, trimZeroString(pd.DeviceID[:]), countValidExtents(pd))
		...
		t.Logf("Disk MD5: %x", md5Disk)

		if md5File != md5Disk {
			t.Fatalf("MD5 mismatch!\n  file: %x\n  disk: %x", md5File, md5Disk)
		}
		...
		if trimZeroString(pds2[0].DeviceID[:]) != "test-disk-001" {
			t.Errorf("device ID mismatch: got %q", trimZeroString(pds2[0].DeviceID[:]))
		}
		t.Logf("Load after Flush: OK, device=%s", trimZeroString(pds2[0].DeviceID[:]))
