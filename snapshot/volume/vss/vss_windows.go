...
func (vss *IVssBackupComponents) Query(objectType VssObjectType) (*IVssEnumObject, error) {
	...
	if runtime.GOARCH == "386" {
		...
		err = syscallNF("Query()", vss.getVTable().query,
...
func (p *VssSnapshot) GetOriginalVolumeName() string {
	return windows.UTF16PtrToString(p.snapshotProperties.originalVolumeName)
}

func (p *VssSnapshot) GetCreateTime() uint64 {
	return p.snapshotProperties.creationTimestamp
}
...
func NewVssSnapshot(provider string,
	volume string, timeout time.Duration, filter VolumeFilter, noWriter bool, msgError ErrorHandler) (VssSnapshot, error) {
...
func DeleteVssSnapshot(snapID string) error {
	snapIDHex := ole.NewGUID(snapID)
	...
	if _, _, e := iVssBackupComponents.DeleteSnapshots(*snapIDHex); e != nil {
		err = newVssTextError(fmt.Sprintf("Failed to delete snapshot(%s): %s", snapIDHex.String(), e.Error()))
		return err
	}
...
func QueryVssSnapshot() ([]Snapshot, error) {
	...
	enum, err := iVssBackupComponents.Query(VSS_OBJECT_SNAPSHOT)
	defer enum.Release()
	if err != nil {
		return nil, err
	}
	...
	for {
		...
		count, e := enum.Next(1, unsafe.Pointer(&props))
		if e != nil {
			return nil, e
		}
		...
		ss = append(ss, Snapshot{
			...
			SnapshotDevice:  props.snapshot.GetSnapshotDeviceObject(),
			CreateTimestamp: props.snapshot.creationTimestamp,
		})

		e = vssFreeSnapshotProperties(&props.snapshot)
		if e != nil {
			return nil, e
		}
	}
