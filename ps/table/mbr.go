...
func (mbr *MBR) Size() (int64, error) {
	size, err := extend.FileSize(mbr.DiskPath)
...
//
// ```
func (mbr *MBR) MarshalJSON() (o []byte, err error) {
	mj := new(MBRJson)
...
//
// ```
func (mbr *MBR) String() (string, error) {
	o, err := mbr.MarshalJSON()
	if err != nil {
		return "", err
	}
	mj := new(MBRJson)
	err = json.Unmarshal(o, mj)
	if err != nil {
		return "", err
	}
	fmt_ := `
