...
func lvmProbe() (bool, error) {
	r, o, eo := command.Execute("lvm version")
	switch r {
	case 127:
		return false, nil
	case 0:
		SupportLVM = true
		noSpaceOs := strings.Split(extend.TrimAllSpace(o), ":")
...
func Pvcreate(device, uuid string) error {
	r, o, eo := command.Execute(CommandStringForPvCreate(device, uuid))
	if r != 0 {
...
func PvscanCache(device string) error {
	r, o, eo := command.Execute(CommandStringForPvScanCache(device))
	if r != 0 {
...
func Pvs(pvPaths ...string) ([]*PhysicalVolume, error) {
	r, o, eo := command.Execute(CommandStringForPvs(pvPaths...))
	if r != 0 {
...
func Pvresize(device string, size int64) error {
	r, o, eo := command.Execute(CommandStringForPvResize(device, size))
	if r != 0 {
...
func Pvremove(device string) error {
	r, o, eo := command.Execute(CommandStringForPvRemove(device))
	if r != 0 {
...
func Vgcreate(name string, physicalextentsizeInBytes int, pvPaths ...string) error {
	r, o, eo := command.Execute(CommandStringForVgCreate(name, physicalextentsizeInBytes, pvPaths...))
	if r != 0 {
...
func Vgs(filterVgNames ...string) ([]*VolumeGroup, error) {
	r, output, err := command.Execute(CommandStringForVgs(filterVgNames...))
	if r != 0 {
...
func Vgrename(oldName, newName string) (*VolumeGroup, error) {
	r, o, eo := command.Execute(CommandStringForVgRename(oldName, newName))
	if r != 0 {
...
func Vgextend(vgName string, pvDevices ...string) error {
	...
	r, o, eo := command.Execute(CommandStringForVgExtend(vgName, pvDevices...))
	if r != 0 {
...
func Vgreduce(vgName string, pvDevices ...string) error {
	...
	r, o, eo := command.Execute(CommandStringForVgReduce(vgName, pvDevices...))
	if r != 0 {
...
func Vgremove(vgName string) error {
	r, o, eo := command.Execute(CommandStringForVgRemove(vgName))
	if r != 0 {
...
func Lvcreate(lvName string, vgName string, lvType LVType, sizeInBytes int64) error {
	r, o, eo := command.Execute(CommandStringForLvCreate(lvName, vgName, lvType, sizeInBytes))
	if r != 0 {
...
func LvThinCreate(name, vg, pool string, size int64) error {
	r, o, eo := command.Execute(CommandStringForThinLVCreate(name, vg, pool, size))
	if r != 0 {
...
func Lvrename(oldName, newName string, vg string) (*LogicalVolume, error) {
	r, o, eo := command.Execute(CommandStringForLvRename(vg, oldName, newName))
	if r != 0 {
...
func Lvremove(lvFullName string) error {
	r, o, eo := command.Execute(CommandStringForLvRemove(lvFullName))
	if r != 0 {
...
func ConvertThinPool(poolName, poolMetadataName string) error {
	r, o, eo := command.Execute(CommandStringForConvertLvToThinPool(poolName, poolMetadataName))
	if r != 0 {
