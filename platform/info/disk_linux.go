...
func QueryOneDisk(diskPath string) (d Disk, err error) {
	...
	name, ok := xutil.FindSymlinkByDeviceName("/dev/disk/by-path", diskPath)
	if ok {
		d.PathId = name
	} else {
		// 兼容VIRTIO总线
		name, ok = xutil.FindSymlinkByDeviceName("/dev/disk/by-id", diskPath)
		if ok {
			d.PathId = name
		}
	}
	...
	d.LogicalSectorSize, err = xutil.DiskLogicalSectorSize(diskPath)
	if err != nil {
		return d, err
	}
	d.PhysicalSectorSize, err = xutil.DiskPhysicalSectorSize(diskPath)
	if err != nil {
		return d, err
	}

	sectors, e := xutil.GetDiskSectors(diskPath)
	if e != nil {
		return d, e
	}
	d.Size = sectors * 512
	...
	d.Vendor, err = xutil.GetDiskVendor(diskPath)
	if err != nil {
		return d, err
	}
	d.Model, err = xutil.GetDiskModel(diskPath)
	if err != nil {
		return d, err
	}
	d.SerialNumber, err = xutil.GetDiskSerialNumber(diskPath)
	if err != nil {
		return d, err
	}
	d.Bus, _ = GetDiskBusType(diskPath)

	d.IsReadOnly, err = xutil.IsDiskReadonly(diskPath)
	if err != nil {
		return d, err
	}
