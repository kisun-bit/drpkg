...
func QueryDisks() (disks []Disk, err error) {
	diskPaths, err := xutil.ListDisksV2()
	if err != nil {
	...
	for _, dp := range diskPaths {
		...
		sad, e := xutil.DiskAlignmentStorage(diskPath)
		if e == nil {
			...
			if errors.Is(e, windows.ERROR_NOT_SUPPORTED) ||
				errors.Is(e, windows.ERROR_INVALID_PARAMETER) ||
				errors.Is(e, windows.ERROR_INVALID_FUNCTION) {
		...
		size, e := xutil.FileSize(diskPath)
		if e != nil {
		...
		diskProperty, e := xutil.DiskProperty(diskPath)
		if e != nil {
		...
		offline, readonly, e := xutil.GetDiskAttr(diskPath)
		if e != nil {
