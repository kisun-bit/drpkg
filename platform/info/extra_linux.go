...
func QueryLinuxRelease(rootDir string) LinuxRelease {
	...
	releaseOutput := xutil.GlobReadFiles(releaseFileGlob)
...
func QuerySwapInfo() (ss []LinuxSwap, err error) {
	...
	for _, line := range strings.Split(string(bs), "\n") {
		...
		s := LinuxSwap{
			...
			Type:     lineItems[1],
			Size:     xutil.MustInt64(lineItems[2]) * 1024,
			Used:     xutil.MustInt64(lineItems[3]) * 1024,
			Priority: int(xutil.MustInt64(lineItems[4])),
		}
		if strings.HasPrefix(s.Filename, "/dev") {
			s.Device = s.Filename
			s.UUID = xutil.MatchDevLinkName("/dev/disk/by-uuid", filepath.Base(s.Filename))
			s.Label = xutil.MatchDevLinkName("/dev/disk/by-label", filepath.Base(s.Filename))
			if strings.HasPrefix(filepath.Base(s.Filename), "dm-") {
				dmName := xutil.MatchDevLinkName("/dev/mapper", filepath.Base(s.Filename))
