...
func (fixer *linuxSystemFixer) Repair() error {
	...
	if fixer.opts.RecoveryParam.SkipDriverRepairIfPlatformUnchanged &&
		fixer.opts.RecoveryParam.Source.Base == defs.HPVirt &&
		fixer.opts.RecoveryParam.Target.Base == defs.HPVirt &&
...
func (fixer *linuxSystemFixer) GetPreferHostConfig(virtual defs.HPVirtType) (cfg PreferConfig, err error) {
...
func (fixer *linuxSystemFixer) mountSys() error {
	...
	if fixer.offsys.devEfi != "" {
		...
		if xutil.IsExisted(efiMountpoint) {
...
func (fixer *linuxSystemFixer) deprecateSysMountPoint() error {
	...
	umountDev := false
	if fixer.offsys.distro.Family == defs.LinuxFamilyRHEL && fixer.offsys.distro.Major <= 4 {
		umountDev = true
	}
...
var initrdTools = []initrdTool{
	{
		name:        defs.InitrdToolDracut,
		featureFile: "/etc/dracut.conf",
		cmd:         defs.InitrdToolDracut,
	},
	{
		name:        defs.InitrdToolUpdateInitramfs,
		featureFile: "/etc/initramfs-tools/update-initramfs.conf",
		cmd:         defs.InitrdToolUpdateInitramfs,
	},
	{
		name: defs.InitrdToolMkinitrd,
		cmd:  defs.InitrdToolMkinitrd,
	},
}
...
func (fixer *linuxSystemFixer) detectDeviceMaps() error {
	...
	if xutil.IsExisted(blkidCacheFile) {
...
func (fixer *linuxSystemFixer) detectKvmCfg() {
	...
	fixer.offsys.kvmVideo = defs.VideoBochs
	// FIXME: 后续不要根据主板去决定显卡类型
	if fixer.offsys.kvmChipset == defs.ChipsetQ35 {
		fixer.offsys.kvmVideo = defs.VideoVGA
	}

	fixer.offsys.kvmDiskBus = defs.DiskBusVirtioScsi
	fixer.offsys.kvmNetworkType = defs.NetworkTypeE1000
...
func (fixer *linuxSystemFixer) fixEfiFirmware() error {
	...
	defaultEfiPath := ""
	if xutil.IsExisted(uefiFallbackPath) {
	...
	if defaultEfiPath == "" {
		...
		if xutil.IsExisted(startupScript) {
...
func (fixer *linuxSystemFixer) fixOneGrub(
	...
	if xutil.IsDir(file) || !xutil.IsExisted(file) {
		return nil
	}
...
func fixGrubBootArgs(distro DistroInfo, content string, disableRaid bool, disableMultipath bool) string {
	...
	if distro.ID == defs.DistroOracleLinux &&
		(distro.Version == "6.5" ||
			distro.Version == "6.6") {
