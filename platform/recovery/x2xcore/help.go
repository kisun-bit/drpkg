...
func DetectFSRepairCmdline(device string) (cmdline string, ok bool) {
	...
	case defs.OsLinux:
		...
		repairCmdMap := map[string]string{
			// ext family
			defs.FsTypeExt2: "e2fsck -y %s",
			defs.FsTypeExt3: "e2fsck -y %s",
			defs.FsTypeExt4: "e2fsck -y %s",

			// xfs
			defs.FsTypeXFS: "xfs_repair -L %s",

			// btrfs
			defs.FsTypeBtrfs: "btrfs check --repair %s",

			// FAT family
			defs.FsTypeFAT:   "fsck.fat -a %s",
			defs.FsTypeVFAT:  "fsck.vfat -a %s",
			defs.FsTypeMSDOS: "fsck.msdos -a %s",

			// ntfs
			defs.FsTypeNTFS: "ntfsfix -b -d %s",

			// special
			defs.FsTypeCramFS: "fsck.cramfs -y %s",
			defs.FsTypeGFS2:   "fsck.gfs2 -y %s",

			// apple
			defs.FsTypeHFS:     "fsck.hfs -y %s",
			defs.FsTypeHFSPlus: "fsck.hfsplus -y %s",

			// unix-like
			defs.FsTypeJFS:      "fsck.jfs -a %s",
			defs.FsTypeMinix:    "fsck.minix -a %s",
			defs.FsTypeReiserFS: "yes Yes | fsck.reiserfs --fix-fixable --rebuild-sb --rebuild-tree -y %s",
		}
...
func (fd *fsDevice) String() string {
	return fmt.Sprintf("%s(%s)", fd.Device, fd.FsType)
}
...
func enumFilesystem(offline []string, luksDeviceList []LuksOpenResult) ([]fsDevice, error) {
	...
	for _, dev := range fsDevList {
		...
		if funk.InStrings(SupportedFsTypes, fsStr) || fsStr == defs.FsTypeSwap {
...
func getUefiArch() string {
	...
	case defs.Arch386:
		tag = "ia32"
	case defs.ArchLoong64:
		tag = "loongarch64"
	case defs.ArchRiscv64:
		tag = "riscv64"
	}
