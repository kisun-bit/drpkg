...
type DistroInfo struct {
	...
	Pretty  string          // 原始字符串
	Source  string          // 来源文件
	Family  defs.OsFamily // 来源：RHEL、ALT、SUSE、DEBIAN
}

func GetFamilyByDistroID(distroID string) defs.OsFamily {
	switch distroID {
	case defs.DistroFedora,
		defs.DistroRHEL,
		defs.DistroCentOS,
		defs.DistroCircle,
		...
		return defs.LinuxFamilyRHEL

	case defs.DistroALTLinux:
		return defs.LinuxFamilyALT

	case defs.DistroSLES,
		defs.DistroSUSEBased,
		defs.DistroOpenSUSE:
		return defs.LinuxFamilySUSE

	case defs.DistroDebian,
		defs.DistroUbuntu,
		defs.DistroLinuxMint,
		defs.DistroKaliLinux:
		return defs.LinuxFamilyDebian
	}
...
func parseIssueFallback(root string) (*DistroInfo, error) {
	...
	case strings.Contains(lower, "ubuntu"):
		info.ID = defs.DistroUbuntu
	case strings.Contains(lower, "debian"):
		info.ID = defs.DistroDebian
	case strings.Contains(lower, "red hat"):
		info.ID = defs.DistroRHEL
	default:
		info.ID = "unknown"
