...
func (fixer *windowsSystemFixer) adaptVirtPlatform() error {
	...
	if param.SkipDriverRepairIfPlatformUnchanged &&
		param.Source.Base == defs.HPVirt &&
		param.Target.Base == defs.HPVirt &&
...
func (fixer *windowsSystemFixer) getUnconfigFunc(virt defs.HPVirtType) func() error {
	switch virt {
	case defs.HPVTXen:
		return fixer.unconfigXen
	case defs.HPVTVmware:
		return fixer.unconfigVmware
	case defs.HPVTKvm:
		return fixer.unconfigKvm
	case defs.HPVTHyperV:
		return fixer.unconfigHyperV
	case defs.HPVTParallels:
...
func (fixer *windowsSystemFixer) getConfigFunc(virt defs.HPVirtType) func() error {
	switch virt {
	case defs.HPVTXen:
...
func (fixer *windowsSystemFixer) GetPreferHostConfig(virtual defs.HPVirtType) (cfg PreferConfig, err error) {
	switch virtual {
	case defs.HPVTKvm:
		cfg.Chipset = defs.ChipsetI440fx
		cfg.Video = defs.VideoVGA
		cfg.DiskBus = defs.DiskBusIde
		cfg.NetworkType = defs.NetworkTypeRTL8192

		if modern, _ := fixer.isModernWindows(); modern {
			cfg.Chipset = defs.ChipsetQ35
			cfg.DiskBus = defs.DiskBusVirtio
			cfg.NetworkType = defs.NetworkTypeVIRTIO
		} else {
...
func (fixer *windowsSystemFixer) detectSysVolume() error {
	...
	logger.Debugf("detectSysVolume: volumes:\n%s", xutil.Pretty(fixer.offsys.volumeLtrList))
...
func (fixer *windowsSystemFixer) detectBootMode() error {
	...
	for _, v := range fixer.offsys.volumeLtrList {
		...
		fixer.offsys.bootMode = defs.BootModeUEFI
		fixer.offsys.efiVolumeLtr = v
		logger.Debugf("detectBootMode: uefi")
		return nil
	}

	fixer.offsys.bootMode = defs.BootModeBIOS
...
func (fixer *windowsSystemFixer) detectHAL() error {
	...
	case strings.Contains(id, "ACPI Multiprocessor"):
		fixer.offsys.halType = defs.HALACPIMultiprocessor
		break

	case strings.Contains(id, "ACPI Uniprocessor"):
		fixer.offsys.halType = defs.HALACPIUniprocessor
		break

	case strings.Contains(id, "Standard PC"):
		fixer.offsys.halType = defs.HALStandardPC
		break

	case strings.Contains(id, "MPS Multiprocessor"):
		fixer.offsys.halType = defs.HALMPSMultiprocessor
		break

	case strings.Contains(id, "MPS Uniprocessor"):
		fixer.offsys.halType = defs.HALMPSUniprocessor
		break

	default:
		fixer.offsys.halType = defs.HALUnknown
		break
	}
...
func (fixer *windowsSystemFixer) injectDriversLegacy(ds *x2xlib.DriverResource) (blockDrv, netDrv string, err error) {
	...
	for _, f := range files {
		...
		if e = xutil.CopyFile(src, dst, 0o666); e != nil {
			return "", "", errors.Wrapf(e, "copy %s -> %s", src, dst)
		}
		equal, e := xutil.FileEqual(src, dst)
		if e != nil {
...
func (fixer *windowsSystemFixer) fixUefi() error {
	...
	if fixer.offsys.bootMode != defs.BootModeUEFI {
		return nil
	}
	...
	// 已经存在且内容一致
	if xutil.IsExisted(fallback) {
		equal, err := xutil.FileEqual(bootmgfw, fallback)
		if err == nil && equal {
	...
	if err := xutil.CopyFile(bootmgfw, fallback, 0755); err != nil {
...
func (fixer *windowsSystemFixer) ntVersion() (defs.NTVersion, error) {
	ntVer, ok := defs.OsNTVersion[fixer.offsys.windowsVersion]
	if !ok {
		return defs.NTUnknown, errors.New("not supported windows version")
	}
	return ntVer, nil
}
...
func (fixer *windowsSystemFixer) injectConfigService() error {
	...
	if err = xutil.CopyFile(serviceSrcExePath, serviceTgtExePath, 0o666); err != nil {
