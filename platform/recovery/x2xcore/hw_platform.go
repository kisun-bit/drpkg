package x2xcore

import (
	"runtime"

	"github.com/kisun-bit/drpkg/defs"
	"github.com/kisun-bit/drpkg/platform/bus/pci/universal"
	"github.com/shirou/gopsutil/v3/cpu"
)

// Platform 表示一个运行环境（源或目标）
type Platform struct {
	// Arch OCI风格的架构标识，即runtime.GOARCH
	Arch string `json:"arch"`

	// CpuVendor CPU的制造商ID，可选字段
	CpuVendor string `json:"cpuVendor"`

	// Base 平台类型：virt / bare-metal
	Base defs.HardwarePlatform `json:"base"`

	// Virt 虚拟化类型（仅当 Base=virt 时使用）
	Virt defs.HPVirtType `json:"virt,omitempty"`

	// PciList PCI设备列表（仅当 Base=bare-metal 时使用）
	PciList []string `json:"pciList,omitempty"`
}

// RuntimePlatform 返回当前所运行系统的硬件平台信息
func RuntimePlatform() (pf Platform, err error) {
	pf.Base = defs.HPUnknown
	pf.Virt = defs.HPVTNone

	pf.Arch = runtime.GOARCH

	ci, err := cpu.Info()
	if err != nil {
		return pf, err
	}
	if len(ci) > 0 {
		pf.CpuVendor = ci[0].VendorID
	}

	pciList, err := universal.ListUniPci()
	if err != nil {
		return pf, err
	}
	for _, pci := range pciList {

		switch {
		case pci.VendorId() == 0x15ad && pf.Base == defs.HPUnknown: // 检查是否是vmware环境
			pf.Base = defs.HPVirt
			pf.Virt = defs.HPVTVmware
			continue
		case pci.VendorId() == 0x5853 && pf.Base == defs.HPUnknown: // 检查是否是xen环境
			pf.Base = defs.HPVirt
			pf.Virt = defs.HPVTXen
			continue
		case pci.VendorId() == 0x1af4 && pf.Base == defs.HPUnknown: // 检查是否是qemu/kvm环境
			pf.Base = defs.HPVirt
			pf.Virt = defs.HPVTKvm
			continue
		}

		// 非虚拟硬件将其添加到pci列表中

		pf.PciList = append(pf.PciList, pci.String())
	}

	// 检查是否是hyper-v环境
	yes, err := VmbusExisted()
	if err != nil {
		return pf, err
	}
	if yes && pf.Base == defs.HPUnknown {
		pf.Base = defs.HPVirt
		pf.Virt = defs.HPVTHyperV
	}

	// 排除所有虚拟化，确定为物理机环境
	if pf.Base == defs.HPUnknown {
		pf.Base = defs.HPBareMetal
		pf.Virt = defs.HPVTNone
	}

	return pf, nil
}
