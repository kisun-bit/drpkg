...
func (fixer *linuxSystemFixer) patchVirtIO() error {
	...
	for _, k := range fixer.offsys.kernels {
		if err := fixer.patchOneKernelVirtIO(k); err != nil {
			// 提示警告，此内核不兼容virtio硬件设备
			fixer.warnf(LogTplForKVMPatchFailedWith1Args, err)
			logger.Warnf(
				"patchVirtIO: patchOneKernelVirtIO: %v, disk-bus changes to `%s`",
				err, defs.DiskBusSata)
			fixer.offsys.kvmDiskBus = defs.DiskBusSata
			return nil
		}
	}
...
func (fixer *linuxSystemFixer) patchOneKernelVirtIO(k kernel) error {
	...
	if supportedNetVirtio {
		fixer.offsys.kvmNetworkType = defs.NetworkTypeVIRTIO
	}
	if supportedDiskVirtio {
		fixer.offsys.kvmDiskBus = defs.DiskBusVirtio
	}
