...
func checkWindowsVersion(windowsVersion defs.WindowsVersion) error {
	if _, ok := defs.OsNTVersion[windowsVersion]; !ok {
...
func getDriverTypeByVirtType(virt defs.HPVirtType) (uint16, error) {
	switch virt {
...
func createNTCompat(
	tx *gorm.DB,
	driverID string,
	windowsVersions []defs.WindowsVersion,
) error {
