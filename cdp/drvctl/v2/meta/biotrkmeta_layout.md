...
对齐粒度作用于区域而非单条记录：`Protected Device Records` 是变长的，
...
四个区域首尾相接，互不重叠，起始偏移与大小都是 4096 的整数倍：
...
ProtectedRegionOffset = BitmapDataOffset + BitmapDataSize
...
1. **增长不会踩到别的区域**。记录区后面没有任何东西，`BitmapExtents` 解析出多少段、
   记录变成多长，都只是把区域末尾往后推。若记录区在 Allocation Map 之前，
   记录一旦超出预留空间就会直接覆盖 Allocation Map 与 Bitmap Unit Data。
2. **`BitmapExtents` 的解析结果稳定**。它描述的是位图数据区的物理分布，
   而位图数据区在记录区之前、创建时就已完整分配且永不移动，
   所以解析结果与记录内容无关，一次写入即可收敛。
3. **不需要容量参数**。Extents / BitmapExtents 的数量没有格式上限，
...
区域缩小时，历史高水位以下的尾部必须一并写零，避免残留已删除设备的记录。
...
| 48 | 8 | `ProtectedRegionOffset` | 记录区起始偏移 |
| 56 | 8 | `ProtectedRegionSize` | 记录区大小，随记录变化 |
| 64 | 4 | `ProtectedDeviceCount` | 记录条数 |
...
| 96 | 4 | `HeaderCRC32` | 整个 4096 Byte Header 的 CRC32 |
| 100 | 3996 | `Reserved` | 保留，填 `0x00` |
...
一个 Bitmap Unit 覆盖的空间  = BitmapClusterSize × 8 × BitIndexSpace   默认 128 GiB
Bitmap Bit 总数              = TotalBitmapUnits × BitmapClusterSize × 8  默认 268,435,456
可寻址的受保护空间总量        = Bitmap Bit 总数 × BitIndexSpace          默认 1 PiB
```

默认值：`BitIndexSpace = 4 MiB`、`BitmapClusterSize = 4096`、`TotalBitmapUnits = 8192`。
...
最后更新设备记录。这样不会出现 `Allocation = 1` 但数据未初始化的中间态。

释放时只需清 Allocation Bit，Unit Data 不要求清零。
允许"已分配但当前无 Extent 引用"（预分配、延迟回收等），
但**有效 Extent 引用的 Unit 必须已分配**。
...
同一个 `ProtectedDevice` 内不同 Extent 的 Bitmap Unit 范围不得重叠；
不同设备之间允许共享同一个 Unit。

---

## 6. Protected Device Records
...
| 4 | 4 | `Type` | DeviceType，见 6.5 |
| 8 | 512 | `DeviceID` | 设备唯一标识，见 6.6 |
| 520 | 4 | `ExtentCount` | 紧随其后的 `ProtectedExtent` 数量 |
| 524 | 变长 | `Extents` | `ExtentCount` 个 `ProtectedExtent`，紧凑排列 |

```text
TotalSize                 = 524 + Σ ProtectedExtent[i] 大小
ProtectedDeviceMinBinSize = 524
```

`TotalSize` 是顺序扫描的唯一依据，读取时必须以它为准。

### 6.2 ProtectedExtent

| 偏移 | 大小 | 字段 | 说明 |
| ---: | ---: | :--- | :--- |
| 0 | 536 | `Extent` | 受保护的磁盘区间（`DiskExtent`） |
| 536 | 8 | `BitmapUnitStart` | 起始 Bitmap Unit 索引 |
| 544 | 8 | `BitmapUnitCount` | Bitmap Unit 数量 |
| 552 | 4 | `BitmapExtentCount` | 紧随其后的 `DiskExtent` 数量 |
| 556 | 变长 | `BitmapExtents` | `BitmapExtentCount` 个 `DiskExtent`，紧凑排列 |

```text
大小 = 556 + BitmapExtentCount × 536
```

Bitmap Unit 范围是左闭右开区间 `[BitmapUnitStart, BitmapUnitStart + BitmapUnitCount)`。

`BitmapExtents` 只有在 Flush 之后才有内容，未解析时 `BitmapExtentCount = 0`。

### 6.3 BitmapExtents

`BitmapExtents` 就是 `DiskExtent` 数组，没有独立的类型，
描述**位图数据本身**在物理磁盘上的位置。字段含义与 `Extent` 不同：

| 字段 | 在 `Extent` 里 | 在 `BitmapExtents[i]` 里 |
| :--- | :--- | :--- |
| `DiskID` | 受保护区间所在的物理磁盘 | 位图数据所在的物理磁盘 |
| `Start` | 受保护区间在该磁盘上的偏移 | 位图数据在该磁盘上的**绝对偏移**，不是元数据区域内的相对偏移 |
...
3. 对每个 `ProtectedExtent`，把它的 Bitmap Unit 范围映射到裁剪后的物理段上，
   逐段生成一个 `DiskExtent`。
...
`ProtectedExtent.Extent.DiskID` 是该受保护区间实际所在的**物理磁盘**。
两者可以不同，因此天然支持跨磁盘卷：
...
```text
空记录（0 个 Extent）                  = 524
1 个 Extent，未解析 BitmapExtents      = 524 + 556          = 1080
1 个 Extent，解析出 1 段 BitmapExtents = 524 + 556 + 536    = 1616
1 个 Extent，解析出 7 段 BitmapExtents = 524 + 556 + 7×536  = 4832
```

3 个设备、每个 1 个 Extent、各解析出 1 段：

```text
Σ TotalSize         = 3 × 1616            = 4848
ProtectedRegionSize = AlignUp4096(4848)   = 8192
TotalSize           = 33562624 + 8192     = 33570816
```
...
默认参数 `BitIndexSpace = 4 MiB`、`BitmapClusterSize = 4096`、`TotalBitmapUnits = 8192`：
...
创建后（尚无设备记录）`TotalSize = 33,562,624 Byte ≈ 32.01 MiB`。
...
BitmapAllocMapSize     == AlignUp4096((TotalBitmapUnits + 7) / 8)
BitmapDataOffset       == BitmapAllocMapOffset + BitmapAllocMapSize
ProtectedRegionOffset  == BitmapDataOffset + BitmapClusterSize × TotalBitmapUnits
...
ExtentCount × 556      <= TotalSize
BitmapExtentCount × 536 <= 所在记录剩余字节数
...
BitmapUnitCount × BitmapClusterSize × 8 × BitIndexSpace >= Extent.Size
同一设备内各 Extent 的 Bitmap Unit 范围互不重叠
```
