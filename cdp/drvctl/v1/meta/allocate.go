...
func fillBitmapRegionByFileExtents(
	...
	allocatedCopy := make([]ioctl2.Segment, len(allocated))
	copy(allocatedCopy, allocated)

	waitAllocateMap := make(map[int][]ioctl2.Segment)
	for pIdx, p := range drvParam.ProtectDisks {
		...
		waitAllocateMap[pIdx] = []ioctl2.Segment{}
	}
...
func buildInitialAllocated(
	...
	allocated := make([]ioctl2.Segment, 0)
	...
	for _, fileE := range fileEs {
		...
		allocated = append(allocated, ioctl2.Segment{
...
func subtractExtents(
	...
	for _, fe := range fileEs {
		...
		for _, a := range allocated {
			...
			if aStart > curStart {
				free = append(free, ioctl2.Segment{
					Start: curStart,
...
func allocateFromFreeExtents(
	free []ioctl2.Segment,
	size uint64,
) ([]ioctl2.Segment, error) {
	left := size
	result := make([]ioctl2.Segment, 0)
	...
	for _, f := range free {
		...
		result = append(result, ioctl2.Segment{
			Start: f.Start,
			Size:  use,
		})
		left -= use
